...
import redis
import pandas as pd
import platform # Importa a biblioteca para detetar o sistema operativo
...
# A UI do Spark só é garantida no modo local
if spark.conf.get("spark.master") == "local" or spark.conf.get("spark.master").startswith("local["):
...
query_globais = metricas_globais.writeStream.outputMode("complete").foreachBatch(lambda df, epoch_id: write_to_redis(df, "metricas_globais")).option("checkpointLocation", f"{CHECKPOINT_BASE_PATH}/globais").trigger(processingTime=TRIGGER_INTERVAL).start()
...
query_categoria = receita_por_categoria.writeStream.outputMode("complete").foreachBatch(lambda df, epoch_id: write_to_redis(df, "receita_por_categoria")).option("checkpointLocation", f"{CHECKPOINT_BASE_PATH}/categoria").trigger(processingTime=TRIGGER_INTERVAL).start()
...
query_top_produtos = contagem_produtos.writeStream.outputMode("complete").foreachBatch(lambda df, epoch_id: processa_e_escreve_top_n(df, epoch_id, n=5)).option("checkpointLocation", f"{CHECKPOINT_BASE_PATH}/top_produtos").trigger(processingTime=TRIGGER_INTERVAL).start()
...
query_logins = total_logins.writeStream.outputMode("complete").foreachBatch(lambda df, epoch_id: write_to_redis(df, "total_logins")).option("checkpointLocation", f"{CHECKPOINT_BASE_PATH}/logins").trigger(processingTime=TRIGGER_INTERVAL).start()
...
query_criados = total_carrinhos_criados.writeStream.outputMode("complete").foreachBatch(lambda df, epoch_id: write_to_redis(df, "total_carrinhos_criados")).option("checkpointLocation", f"{CHECKPOINT_BASE_PATH}/carrinhos_criados").trigger(processingTime=TRIGGER_INTERVAL).start()
...
query_convertidos = total_carrinhos_convertidos.writeStream.outputMode("complete").foreachBatch(lambda df, epoch_id: write_to_redis(df, "total_carrinhos_convertidos")).option("checkpointLocation", f"{CHECKPOINT_BASE_PATH}/carrinhos_convertidos").trigger(processingTime=TRIGGER_INTERVAL).start()
