...
def main():
    ...
    tv_raw_df = bronze_jobs.read_from_postgres_with_partition(spark, jdbc_url, db_properties, 'transacoes_vendas', 'id_transacao', NUM_PARTITIONS)
    # Ler clientes
    print("\nIniciando leitura da tabela de clientes...")
    dc_raw_df = bronze_jobs.read_from_postgres_with_partition(spark, jdbc_url, db_properties, 'dados_clientes', 'id_usuario', NUM_PARTITIONS)
    # Ler eventos web
    print("\nIniciando leitura da tabela de eventos web...")
    ew_raw_df = bronze_jobs.read_from_postgres_with_partition(spark, jdbc_url, db_properties, 'eventos_web', 'id_evento', NUM_PARTITIONS)
    # Ler catálogo de produtos
    print("\nIniciando leitura da tabela de catálogo de produtos...")
    cp_raw_df = bronze_jobs.read_from_postgres_with_partition(spark, jdbc_url, db_properties, 'catalogo_produtos', 'id_produto', NUM_PARTITIONS)

    # Limpar dados
    print("\nIniciando limpeza dos dados...")
    tv_cleaned_df = bronze_jobs.clean_transactions_data(tv_raw_df)
    dc_cleaned_df = bronze_jobs.clean_clients_data(dc_raw_df)
    ew_cleaned_df = bronze_jobs.clean_web_events_data(ew_raw_df)
    cp_cleaned_df = bronze_jobs.clean_products_data(cp_raw_df)
    ...
    delta_base_path = bronze_jobs.deltalake_bronze_path
    delta_base_path.mkdir(parents=True, exist_ok=True) # Garante que o diretório exista

    print("\nIniciando escrita para o Delta Lake...")
    bronze_jobs.write_to_delta(tv_cleaned_df, str(delta_base_path / "transacoes_vendas"))
    bronze_jobs.write_to_delta(dc_cleaned_df, str(delta_base_path / "dados_clientes"))
    bronze_jobs.write_to_delta(ew_cleaned_df, str(delta_base_path / "eventos_web"))
    bronze_jobs.write_to_delta(cp_cleaned_df, str(delta_base_path / "catalogo_produtos"))
